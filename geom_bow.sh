...
cd geom_vocsearch
bash run.sh
./a.out $TEST_DIR/list.db.binary.txt $TEST_DIR/list.query.binary.txt $TEST_DIR $TEST_DIR/bundle/bundle.db.out $TEST_NUM > ../../results/$TEST_NAME/$TEST_NAME.geom.bow.$TEST_NUM 

cd ../generate_bow/
python convert_result.py ../../results/$TEST_NAME/$TEST_NAME.geom.bow.$TEST_NUM $TEST_DIR/list.db.images.txt $TEST_DIR/list.query.images.txt $TEST_DIR/bundle/list.orig.txt > ../../results/$TEST_NAME/$TEST_NAME.geom.bow.$TEST_NUM.orig $TEST_NUM
